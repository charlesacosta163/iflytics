...
import { getUserStats, matchATCRank, getPilotServerSessions } from "@/lib/actions";
import { FaStar, FaPlane, FaPlaneDeparture, FaArrowLeft } from "react-icons/fa";
          ...
          <div className="flex-1 flex flex-col gap-4 max-w-[500px] w-full px-4 bg-white rounded-lg shadow p-8">
            {pilotServerSession ? (
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-green-600">Flying</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-3 bg-blue-50 rounded-lg">
                    ...
                    <p className="font-bold text-blue-600">{pilotServerSession.callsign}</p>
                  </div>
                  <div className="text-center p-3 bg-green-50 rounded-lg">
                    <p className="text-xs text-gray-600">Altitude</p>
                    <p className="font-bold text-green-600">{Math.round(pilotServerSession.altitude).toLocaleString()} ft</p>
                  </div>
                  <div className="text-center p-3 bg-purple-50 rounded-lg">
                    <p className="text-xs text-gray-600">Speed</p>
                    <p className="font-bold text-purple-600">{Math.round(pilotServerSession.speed)} kts</p>
                  </div>
                  <div className="text-center p-3 bg-orange-50 rounded-lg">
                    <p className="text-xs text-gray-600">Heading</p>
                    <p className="font-bold text-orange-600">{Math.round(pilotServerSession.heading)}°</p>
                  </div>

                  {
                    pilotServerSession.isConnected ? (
                      <div className="col-span-2 bg-gray-100 text-gray p-3 rounded-lg flex items-center justify-center gap-2">
                        <b className="text-white font-bold flex items-center gap-1">Currently Online <FaPlus className="w-4 h-4"/></b>
                      </div>
                    ) : (
                      <div className="col-span-2 bg-blue-400 text-white p-3 rounded-lg flex items-center justify-center gap-2">
                    <b className="text-white font-bold flex items-center gap-1">Powered by Autopilot Plus <FaPlus className="w-4 h-4"/></b>
                  </div>)
                  }
                </div>
              </div>
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold text-gray-600">Not Flying</h3>
                <p className="text-sm text-gray-500">User is not currently flying</p>
              </div>
            )}
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-600 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-white">
                      {matchATCRank(userStats.atcRank.toString())}
                    </p>
                  </div>
                </div>
              </div>
